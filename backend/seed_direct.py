...
DB_PATH = os.path.join(os.path.dirname(__file__), "minicrm.db")
...
NOTIZEN = {
    ...
    "todesfall":        "Risikolebensversicherung läuft bis 2040, Wunschkapital 200k",
}
...
def run():
    ...
    for i, k in enumerate(KUNDEN):
        cols = ", ".join(k.keys())
        placeholders = ", ".join(f":{c}" for c in k.keys())
        conn.execute(f"INSERT INTO customers ({cols}) VALUES ({placeholders})", k)
        cid = conn.execute("SELECT last_insert_rowid()").fetchone()[0]

        pool = STATUS_POOLS[i]
        for cat in CATEGORIES:
            if cat in pool["besprochen"]:
                status, notiz = "besprochen", NOTIZEN.get(cat, "")
            elif cat in pool["offen"]:
                status, notiz = "offen", ""
            else:
                status, notiz = "nicht_besprochen", ""
            conn.execute("""
                INSERT OR REPLACE INTO customer_categories (customer_id, category, status, notizen)
                VALUES (?, ?, ?, ?)
            """, (cid, cat, status, notiz))

        conn.execute("""
            INSERT INTO appointments (customer_id, datum, notizen)
            VALUES (?, '2026-06-15', 'Erstgespräch – Stammdaten + Bedarfsanalyse')
        """, (cid,))

        print(f"OK {k['p1_vorname']} {k['p1_nachname']} (ID {cid})")

    conn.commit()
    conn.close()
    print("\nFertig! 10 Kunden angelegt.")
